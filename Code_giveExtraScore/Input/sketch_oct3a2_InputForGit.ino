...
// String inputText = "Hello"; // по умолчанию
String inputText = ""; // по умолчанию
String bitStream;           // поток битов для трансляции

// Настройки точки доступа
const char* ssidAP = "TMOLzxc";  // Имя WiFi сети
const char* passwordAP = "123456789"; // пароль, минимум 8 символов
...
void handleRoot() {
  String html = R"(<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>форма отправки сообщения</title><style>body{margin:0;height:100vh;display:flex;justify-content:center;align-items:center;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;background:linear-gradient(135deg,#ededed,#e74d3cdf);color:#333}form{background:#fff;padding:30px 20px;border-radius:15px;box-shadow:16px 16px 32px rgba(0,0,0,.685);max-width:500px;width:90%;display:flex;flex-direction:column;gap:20px}h1{margin:0;text-align:center;font-size:22px;color:#444}label{font-weight:600;display:flex;flex-direction:column;font-size:16px;color:#222}.message-input{width:100%;padding:12px 16px;margin-top:8px;border:2px solid #aaa;border-radius:16px;font-size:14px;transition:border-color .2s,box-shadow .2s;box-sizing:border-box;min-height:100px;resize:vertical;box-shadow:0 0 2px rgba(0,0,0,.685)}.message-input:focus{border-color:#e74c3c;outline:0;box-shadow:0 0 8px rgba(231,76,60,.6)}.radio-group{display:flex;flex-wrap:wrap;justify-content:space-around;padding:10px 0;background:#f0f0f0;border-radius:10px;box-shadow:8px 8px 16px rgba(0,0,0,.685)}.radio-label{display:flex;align-items:center;font-size:14px;cursor:pointer;margin:5px auto;transition:transform .2s;color:#222}.radio-text{padding-top:2px}.radio-label:hover{transform:scale(1.05)}.radio-label input[type=radio]{display:none}.radio-custom{width:20px;height:20px;border:2px solid #555;border-radius:50%;margin-right:10px;position:relative;transition:all .2s}.radio-custom::after{content:"";position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:12px;height:12px;border-radius:50%;background-color:transparent;transition:background-color .2s}input[type=radio]:checked+.radio-custom{border-color:#e74c3c}input[type=radio]:checked+.radio-custom::after{background-color:#e74c3c}button{padding:14px;font-size:16px;background:#e74c3c;border:none;border-radius:10px;color:#fff;cursor:pointer;box-shadow:8px 8px 16px rgba(0,0,0,.685);transition:background .3s,transform .2s}button:hover{transform:scale(1.05);background:#c0392b}.radio-item{display:flex;align-items:center;margin-bottom:8px;font-size:15px;font-weight:700}@media (max-width:600px){h1{font-size:20px}label{font-size:16px}.message-input{font-size:15px;padding:14px 18px;min-height:120px}.radio-group{display:flex;flex-direction:column;align-items:center}.radio-label{margin-bottom:8px}}</style><script>document.getElementById('myForm').addEventListener('submit', function(e) {
      e.preventDefault(); // отменить стандартную отправку
      const form = this;
      const formData = new FormData(form);
      fetch('/send', {
...
void handleSend() {
  if (server.hasArg("textInput")) {
    ...
    // server.send(200, "text/plain", "Текст получен: " + inputText);
